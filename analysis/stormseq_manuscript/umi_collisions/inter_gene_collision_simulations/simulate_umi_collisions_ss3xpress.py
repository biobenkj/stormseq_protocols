...
def parse_arguments():
    ...
    parser.add_argument("--log_file", type=str, default="simulation.log", help="Log file of simulation results.")
...
def generate_all_umis(length):
    bases = ['A', 'C', 'G', 'T']
    valid_endings = {'AT', 'TT', 'TA', 'AA'}
    all_umis = [''.join(p) for p in itertools.product(bases, repeat=length)]
    filtered_umis = [umi for umi in all_umis if umi[-2:] in valid_endings]
    return filtered_umis
...
def main():
    ...
    for mu, theta in zip(mu_values, theta_values):
        if use_poisson and np.isclose(mu, theta, rtol=0.1):
            ...
            p = theta / (mu + theta)
            read_depths.append(nbinom.rvs(theta, p, size=1)[0])
    ...
    for gene in range(num_genes):
        ...
        if count > 0:
            sampled_umis = np.random.choice(all_umis, size=count, replace=True)
    ...
    logging.info(f"Total UMIs generated: {total_umis}")
